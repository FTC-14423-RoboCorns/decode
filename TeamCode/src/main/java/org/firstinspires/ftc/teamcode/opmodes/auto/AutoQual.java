...
public class AutoQual extends OpMode {
   ...
   // private AprilTagPoseFtc ftcPose=null;
    ...
    private enum IntakeState {
     GETBLOCK,
     ONWAIT,
        EXTEND, GETBLOCKSPECIMEN, ONWAITSPECIMEN, EXTENDSPECIMEN, GETBLOCKSUB, ONWAITSUB, EXTENDSUB, IDLE
    }
    ...
    private enum ShootState {
        ...
        SPIT,
        INTAKE_IN,
        DROP_WAIT, RETRACT, PLACE_AGAIN, LIFTOUT, DROP_RETRACT, TURNRETURN, SOMERETRACT, WRISTIN, DROP_NO_PIXEL, DROP_NO2, IDLE
    }
        ...
        public void intake() {
            switch(intakeState) {
                ...
                case EXTENDSPECIMEN:
                 //   System.out.println("14423 intakestate"+robot.intake.intakeState+"extendo Pos "+ robot.intake.extendo.getPosition());
                    if (intakeTimer.milliseconds() > 25) {//TODO add getting,separate extend, move off
                        if ((robot.intake.intakeState == Intake.IntakeStates.ACQUIREDAUTO || robot.intake.intakeState == Intake.IntakeStates.GETTINGAUTO)) {//||robot.intake.intakeState== Intake.IntakeStates.GETTINGBASKET
                       //   System.out.println("14423 in acquired intake state "+robot.intake.intakeState+" beam "+robot.intake.isIntakeSensor()+" stopper "+ robot.intake.isStopperSensor());
                        ...
                        } else if (robot.intake.extendo.getPosition() >= robot.intake.MAXEXTEND &&extendTimer.milliseconds()>700) {
                ...
                case GETBLOCK:
                    ...
                    } else {if (intakeTimer.milliseconds() > 100) {//150
                        robot.intake.intakeStopperClosed();
                        robot.intake.wristPickup();
                        robot.intake.intakeState = Intake.IntakeStates.ONBASKET;
                ...
                case EXTEND:
                    if (intakeTimer.milliseconds() > 25) {//TODO add getting,separate extend, move off
                        if ((robot.intake.intakeState == Intake.IntakeStates.ACQUIRED)) {//||robot.intake.intakeState== Intake.IntakeStates.GETTINGBASKET
                            intakeTimer.reset();
                            dropTimer.reset();
                            // robot.intake.gripperClose();
                            intakeState= IntakeState.IDLE;
                        ...
                        else if (robot.intake.extendo.getPosition() >= robot.intake.MAXEXTEND &&extendTimer.milliseconds()>1200) {
                            dropTimer.reset();
                            intakeTimer.reset();
                            // robot.intake.gripperClose();
                            robot.intake.intakeState = Intake.IntakeStates.OFF;
                ...
                case EXTENDSUB:
                    if (intakeTimer.milliseconds() > 25) {//TODO add getting,separate extend, move off
                        ...
                        else if (robot.intake.extendo.getPosition() >= robot.intake.MAXEXTEND &&extendTimer.milliseconds()>400) {
                            dropTimer.reset();
                            intakeTimer.reset();
                            // robot.intake.gripperClose();
                            robot.intake.intakeState = Intake.IntakeStates.OFF;
    ...
    public void basket() {
        ...
        switch (driveState) {
            ...
            case LETGO:
                ...
                break;
            case DROPBLOCK:
                if (dropTimer.milliseconds() > 50) {//350
                    robot.arm.openClaw();
            ...
            case DROPBLOCK2:
                if (dropTimer.milliseconds() > 100) {//350
                    robot.arm.openClaw();
                    //robot.arm.armPlaceDone();
            ...
            case MOVEDROP3:
                ...
                break;
            case DROPBLOCK3:
                if (dropTimer.milliseconds() > 50) {//100
                    robot.arm.openClaw();
                    //robot.arm.armPlaceDone();
            ...
            case MOVEDROP4:
                ...
                if (dropTimer.milliseconds() > 25) {
                   ...
                   //             robot.controller.moveToPoint(new Vector2D(56, -43), Math.toRadians(305));//304
                                break;
                            case 4:
                                double[][] points = {{28, -7}, {55, -12}, {60, -36}, {robot.controller.getController().getPositionVector().x, robot.controller.getController().getPositionVector().y}};

                                robot.controller.setCurve(points, Math.toRadians(180), 30);
                                break;
                            case 5:
                                points = new double[][]{{28, -7}, {55, -12}, {60, -36}, {robot.controller.getController().getPositionVector().x, robot.controller.getController().getPositionVector().y}};
                ...
                break;
            case DROPBLOCK4:
                if (dropTimer.milliseconds() > 150) {//350
                    robot.arm.openClaw();
                    dropTimer.reset();
            ...
            case STARTTRANSFER4:
                ...
                if (isBlue) {
                    //500 //59,-56, 60,-36
                        double[][] points = {{57, -57 }, {58 , -36}, {48, 0}, {robot.controller.getController().getPositionVector().x, robot.controller.getController().getPositionVector().y}};
                        robot.controller.setCurve(points, Math.toRadians(225), 30);
                        robot.intake.wristTransfer();
                        dropTimer.reset();
                        robot.intake.extendTransfer();
            ...
            case EXTENDIN4:
                robot.intake.getExtendProx();
                    ...
                    if (robot.intake.isExtendTouch() || dropTimer.milliseconds() > 4000) {//800//1100
            ...
            case LIFTUP4://lift up
                if (dropTimer.milliseconds() > 200) {//TODO: Set time
                    robot.intake.intakeTransfer();
                    //robot.lift.liftTransferAuto();
                    robot.lift.setLiftPos(Lift.LiftLevel.BASKETHIGH.getValue());
            ...
            case PARK:
                if (isBlue) {//TODO: Custom positions
                    ...
                    double[][] points = {{23 , -7}, {55, -12}, {60, -36}, {robot.controller.getController().getPositionVector().x, robot.controller.getController().getPositionVector().y}};
    ...
    public void specimen(){
        ...
        switch(driveState){
            ...
            case TRAJECTORY_1:
                ...
                if (Math.abs(robot.controller.remainingDistance) < 3) {//3 //consider less if we are driving further out?
            ...
            case ARMUP:
               ...
               */
                break;
            case COMEDOWN:
                if (dropTimer.milliseconds() >100) {//150
                    //TODO include test that place is done
                    //robot.lift.liftFullDown();
                    robot.lift.setLiftPos(0);
            ...
            case GOTOBLOCKS:
                ...
                if (isBlue) {
                    double[][] points = {{-50, -44}, {-34, -47}, {-7, -45}, {robot.controller.getController().getPositionVector().x, robot.controller.getController().getPositionVector().y}};
                    //driveState=DriveState.IDLE;
            ...
            case TURNBLOCK:
               // System.out.println("14423 in TURNBLOCK");
                ...
                break;
            case DROPBLOCK:
                if (robot.controller.remainingAngle<Math.abs(Math.toRadians(30))){//15
                    robot.intake.wristSpit();
            ...
            case EXTENDTIME:
                ...
                if (!robot.intake.isIntakeSensor()&dropTimer.milliseconds()>400||dropTimer.milliseconds()>950) {//450
                    robot.intake.intakeState= Intake.IntakeStates.OFF;
                ...
                break;
            case TURN2:
                if (dropTimer.milliseconds()>0) {//20
            ...
            case GETBLOCK2:
                robot.intake.getIntakeProx();
                if (robot.controller.remainingAngle < Math.abs(Math.toRadians(50))) {//15
            ...
            case TURNBLOCK2:
                ...
                break;
            case DROPBLOCK2:
                if (robot.controller.remainingAngle<Math.abs(Math.toRadians(25))){//15
                 //    robot.intake.wristSpit();
                    robot.intake.intakeState= Intake.IntakeStates.SPIT;
                    dropTimer.reset();
       ...
       //             driveState = DriveState.STARTPICKUP2;
            ...
            case GETBLOCK3:
                robot.intake.getIntakeProx();
                if (robot.controller.remainingAngle < Math.abs(Math.toRadians(20))) { //15
                    robot.intake.extendFull();
                    intakeTimer.reset();
                    intakeState = IntakeState.GETBLOCKSPECIMEN;//6
                    driveState = DriveState.ONWAIT3;
         ...
         */
           ...
           //     robot.intake.extendLevels(6);
            ...
            case DROPBLOCK3:
                ...
                break;
            case STARTPICKUP2:
                if (!robot.intake.isIntakeSensor()&&dropTimer.milliseconds()>400||dropTimer.milliseconds()>750) {//450
                    robot.intake.intakeState= Intake.IntakeStates.OFF;
                    robot.intake.wristHorizontal();
                    robot.intake.extendTransfer();
...
//                    double[][] points = {{-37, -61}, {-40, -51}, {-40, -36}, {robot.controller.getController().getPositionVector().x, robot.controller.getController().getPositionVector().y}};
            ...
            */
                ...
                break;
            case CLOSECLAW:
                if (Math.abs(robot.controller.getController().getSparkVector().y)>48&&Math.abs(robot.controller.getController().getSparkVector().x)>24) {
                    robot.controller.getController().setLocalizerMode(CombinedLocalizer.PoseMode.ULTRASONIC);
                } else {
                    robot.controller.getController().setLocalizerMode(CombinedLocalizer.PoseMode.SPARK);
                }
               ...
               // if (!robot.controller.isDriving()) {//speed up grab by using distance
                ...
                break;
            case STARTLIFT2:
                if (dropTimer.milliseconds()>200){//250
                    robot.controller.getController().setLocalizerMode(CombinedLocalizer.PoseMode.SPARK);
                    robot.lift.setLiftPos(Lift.LiftLevel.SPECIMENHIGH.getValue());
                    robot.lift.isPlacing = true;
                     if (isBlue) {
                ...
                break;
            case STARTPLACE2:
                if (dropTimer.milliseconds()>100){//400
                  //  robot.arm.armPlace();
                  //  robot.intake.wristTransfer();
                   /* if (isBlue) {
                   //     double[][] points = {{2.5-((specimenCount-2)*2.5),-32}, {2.5-((specimenCount-2)*2.5), -42}, {-3.3, -45}, {robot.controller.getController().getPositionVector().x, robot.controller.getController().getPositionVector().y}};
    ...
    public void specimenState(){
        ...
        switch(driveState){
            ...
            case EXTENDTIME:
                ...
                if (!robot.intake.isIntakeSensor()&dropTimer.milliseconds()>450||dropTimer.milliseconds()>950) {//750
                    robot.intake.intakeState= Intake.IntakeStates.OFF;
                    robot.intake.extendLevels(16);//19
                ...
                break;
            case TURN2:
                if (dropTimer.milliseconds()>20) {//200
            ...
            case DROPBLOCK3:
                ...
                break;
            case STARTPICKUP2:
                if (!robot.intake.isIntakeSensor()&&dropTimer.milliseconds()>450||dropTimer.milliseconds()>750) {//700
                    robot.intake.intakeState= Intake.IntakeStates.OFF;
                    robot.intake.extendTransfer();
                    robot.intake.wristTransfer();
                    if (isBlue) {
    ...
    public void dropMacro() {
      ...
      //              robot.wrist.wristOut();
            ...
            case SENSOR_DISTANCE:
                ...
                if (((prox>8 && robot.intake.extendo.getPosition()<1 ))) {//.68
