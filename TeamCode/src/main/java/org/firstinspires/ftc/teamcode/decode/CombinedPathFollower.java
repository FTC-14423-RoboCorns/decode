...
public class CombinedPathFollower {
    ...
    public void updateLine(double robotAngle) {
        ...
        if (resultant.magnitude() < 1) {
            ...
            Vector2D linearVelocityVector = Vector2D.fromHeadingAndMagnitude(
                resultant.angle(),
                calculateLinearSpeedPID()
            ); //calculateVelocity()- controller.getAngularVelocity() * 0.5
    ...
    */
    ...
    public boolean isAtSetPoint(double robotAngle) {
        ...
        if (loose) {
            if (
                controller.getPositionVector().subt(new Vector2D(points[0][0], points[0][1])).magnitude() < 3.5 &&
                Math.abs(calculateError(targetAngle, robotAngle)) < Math.toRadians(5)
            ) {
                //1 and 2
            ...
            if (
                controller.getPositionVector().subt(new Vector2D(points[0][0], points[0][1])).magnitude() < .5 &&
                Math.abs(calculateError(targetAngle, robotAngle)) < Math.toRadians(1)
            ) {
                //1 and 2
                //  if(resultant.magnitude() < 1 && Math.abs(calculateError(targetAngle, robotAngle)) < Math.toRadians(2)){//1.5 and 3
                return true;
            ...
            if (
                controller.getPositionVector().subt(new Vector2D(points[0][0], points[0][1])).magnitude() < 1.5 &&
                Math.abs(calculateError(targetAngle, robotAngle)) < Math.toRadians(3)
            ) {
                //1 and 2
                //  if(resultant.magnitude() < 1 && Math.abs(calculateError(targetAngle, robotAngle)) < Math.toRadians(2)){//1.5 and 3
                return true;
    ...
    public void updateTime() {
        ///THIS IS YOUR BUG! If the pods are not aligned you are resetting your time!!!!!
 ...
 */
    ...
    public double radius(double t) {
        ...
        double[] dd = second(t);
        double rad =
            (Math.pow(Math.pow(d[0], 2) + Math.pow(d[1], 2), (3 / 2))) / Math.abs((d[0] * dd[1]) - (d[1] * dd[0]));
        return rad;
    }
    ...
    public double calculateVelocity() {
        ...
        if (resultant.magnitude() < .5) {
            return 0;
        } else if (resultant.magnitude() > 20) {
            //48
            isTurning = false;
            return 0.3; //.3
        } else {
