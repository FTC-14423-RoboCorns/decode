    ...
    public void localizerUpdate() {
        ...
        if (poseMode == PoseMode.APRILTAG) {
            ...
            aprilTagVector = new Vector2D(
                convertMetersToInches(robot.limelightResult.getBotpose_MT2().getPosition().x),
                -convertMetersToInches(robot.limelightResult.getBotpose_MT2().getPosition().y)
            );
        ...
        if (poseMode == PoseMode.ULTRASONIC) {
            ...
            ultrasonicVector = new Vector2D(
                robot.ultrasonicLocalizer.getPoseEstimate().x,
                robot.ultrasonicLocalizer.getPoseEstimate().y
            );
            ...
            ultrasonicVector = new Vector2D(
                robot.ultrasonicLocalizer.getDiagonalPoseEstimate().x,
                robot.ultrasonicLocalizer.getDiagonalPoseEstimate().y
            );
